            ...
            <div class="relative sm:max-w-sm w-full">
                ...
                <div class="relative w-full rounded-3xl  px-6 py-4 bg-gray-100 shadow-md">
                    ...
                    <form  @submit.prevent="onSubmit" class="mt-10">
                        ...
                        <div>
                            <input v-model="form.email" type="email" placeholder="Correo electronico" class="mt-1 block w-full border-none bg-gray-100 h-11 rounded-xl shadow-lg hover:bg-blue-100 focus:bg-blue-100 focus:ring-0">
                        </div>
            
                        <div class="mt-7">                
                            <input v-model="form.password" type="password" placeholder="Contraseña" class="mt-1 block w-full border-none bg-gray-100 h-11 rounded-xl shadow-lg hover:bg-blue-100 focus:bg-blue-100 focus:ring-0">                           
                        </div>
...
export default {
  ...
  data: () => ({
   ...
   methods: {
        onSubmit() {
            this.$http.post("/api/v1/user/login", this.form).then((res) => {
                 console.log(res);
            })
